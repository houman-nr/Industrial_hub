...
(function(){
    ...
    var chat = {
        ...
        cacheDOM: function() {
            ...
            this.$chatHistoryList = this.$chatHistory.find('ul');
            this.$contactsList = $('.list');
            this.$chatHeaderImg = $('.chat-header img');
            this.$chatHeaderName = $('.chat-header .chat-with');
            this.$chatHeaderStatus = $('.chat-header .chat-num-messages');
        },
        bindEvents: function() {
            this.$button.on('click', this.addMessage.bind(this));
            this.$textarea.on('keyup', this.addMessageEnter.bind(this));
        ...
        render: function() {
            ...
            if (this.messageToSend.trim() !== '') {
                ...
                this.$chatHistoryList.append(template(context));
                // Add the message to the current contact's message history
                if (!messages[this.selectedContact]) {
                  messages[this.selectedContact] = [];
              }
              messages[this.selectedContact].push({
                  sender: 'You',
                ...
                this.updateChatHeader(this.selectedContact); // Update the header with the latest message count
            }
        },
        ...
        loadMessages: function(contactName) {
            ...
            contactMessages.forEach(msg => {
                ...
                if (msg.type === 'outgoing') {
                    ...
                    this.$chatHistoryList.append(Handlebars.compile($("#message-response-template").html())({ response: msg.message, sender: msg.sender,time: msg.time }));
                }
            });

            this.scrollToBottom();
            this.updateChatHeader(contactName);
        },
        updateChatHeader: function(contactName) {
            const contact = contacts.find(c => c.name === contactName);
            if (contact) {
                this.$chatHeaderImg.attr('src', contact.img);
                this.$chatHeaderName.text(contact.name);
                this.$chatHeaderStatus.text(`${messages[contactName].length} پیام`); // You can update the message count if needed
            }
        },
